...
class Motion {
  ...
  _mouseActions() {
    ...
    if ( this.on_hover_objs ) {
      ...
      this.container.delegate( this.on_hover_objs, 'mouseleave', () => {
    ...
    if ( this.on_hover === 'pause' ) {
      this.container.delegate( '.hoverAction', 'mousemove', () => {
        this.is_paused = true;
        clearTimeout( this.delay_timer );
        this.delay_timer = setTimeout( () => {
          this.is_paused = false;
  ...
  _isNeedToAnimate( elem ) {
    const compare_elem = elem || this.elem;
    const content_size = this._setContentLength( compare_elem );
    let parent_size = 0;
    if ( this.axis === 'horizontal' ) {
