...
function formatDate( date ) {
  ...
  const mm = month_text[ date.getMonth() ];
...
class WorkList {
  ...
  getWorks( type, lines ) {
    ...
    if ( type === 'mobile' ) {
      ...
      const works_copy = this.works.slice( 0 );

      for ( let i = 0; i < lines.data_lines.length; i++ ) {
        const row = works_copy.splice( 0, lines.data_lines[ i ] ); // cколько срезаем для линии
        const func = `_${lines.line_types[ i ]}`;
...
class Lines {
  ...
  _init() {
    ...
    if ( this.clear_part > 0 ) {
      for ( let i = 0; i < this.clear_part; i++ ) {
  ...
  _setLineTypesTrio() {
    ...
    const lines_count = this.data_lines.length;
    this.trio_type_tmp = '';
    this.data_lines = [];
    this.line_types = [];
  ...
  _selectLineType( variants = 3 ) { // Выбираем рандомную комбинацию из трех элементов
    let typeID = 0;
    do {
...
const controller = new Controller( textData );
